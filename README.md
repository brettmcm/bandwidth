...
Double-click `Open Bandwidth.command`. It starts Bandwidth locally when needed and
opens `http://127.0.0.1:3000` in your default browser. Bandwidth listens only on this Mac.
The local services stay available in the background until they are stopped or the Mac restarts.

The browser interface uses a second loopback-only endpoint to read Daily Notes directly from the
canonical Deep Thought vault. Daily Note Markdown remains in memory and is never copied into D1 or
browser storage.
...
context. The day's `End Of Day Brief` is intentionally reserved for the **Logs** tab.

Today is read-only. Daily Note Markdown is held in memory by the loopback Daily Notes reader (or
the optional native app) and is never copied into D1 or browser storage. Returning to Bandwidth or
using Refresh picks up the latest note.

## Logs

The **Logs** tab is the historical companion to the future-facing runway. Beginning July 6, 2026, it shows one month at a time. A dot marks each date with a populated `End Of Day Brief`; selecting any available date opens a familiar right-side drawer with two records:
...
Daily Note Markdown is read only by the loopback reader, held in memory, and never copied into D1
or browser storage. Bandwidth resolves Deep Thought through `~/.codex/obsidian-vaults.json` and
Obsidian's configured Daily Notes folder; editing continues in Obsidian. Non-local browser previews
retain Asana history but show that local notes are unavailable.
...
`npm run dev` starts both the browser app on port 3000 and the local Daily Notes reader on port
3001. Set `BANDWIDTH_PORT` to move them to another adjacent pair of loopback ports.

`npm start` uses the same local runtime. `npm run build` remains available to validate the hosted
worker build, but the browser launcher does not need to rebuild Bandwidth before opening it.
...
- Lifecycle: the app starts the same local server and opens the runway in WebKit. The browser launcher is now the default.
...
- Icon source: `native/Bandwidth/Resources/kairos-icon.icon` (also rendered as `public/favicon.png`)
